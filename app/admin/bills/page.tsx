...
function receiptText(b: Bill) {
  return [
    'Travel Story', 'SALES RECEIPT', `Bill No: ${b.billNo}`, `Date: ${b.date}`,
    `Customer: ${b.customer.name}`, `Mobile: ${b.customer.mobile}`, `Place: ${b.customer.place || '-'}`, '',
    ...b.items.map(i => `${i.name} x ${i.qty} = ${money(i.total)}`), '',
    `Subtotal: ${money(b.subtotal)}`,
    ...(b.discount > 0 ? [`Discount: ${money(b.discount)}`] : []),
    `Total Amount: ${money(b.total ?? b.subtotal)}`, `Paid: ${money(b.paid)}`, `Balance: ${money(b.balance)}`, `Payment: ${b.payment}`
  ].join('\n');
}
...
function downloadExcel(rows: Bill[], from: string, to: string) {
  ...
  const a = document.createElement('a'); a.href = url; a.download = `travel-story-bills-${from}-to-${to}.xls`; a.click(); URL.revokeObjectURL(url);
}
...
export default function Bills() {
  ...
  const [from, setFrom] = useState(() => monthBounds()[0]);
  const [to, setTo] = useState(() => monthBounds()[1]);
  const [copyMessage, setCopyMessage] = useState(false);

  const data = useMemo(() => {
    const q = query.toLowerCase();
    return bills.filter(b => `${b.billNo} ${b.customer.name} ${b.customer.mobile}`.toLowerCase().includes(q) && b.date >= from && b.date <= to);
  }, [bills, query, from, to]);

  async function copyBill(b: Bill) {
    try { await navigator.clipboard.writeText(receiptText(b)); setCopyMessage(true); toast('Bill copied to clipboard'); window.setTimeout(() => setCopyMessage(false), 2500); }
  ...
  return <>
    ...
    <Panel>
      ...
      <div className="table-card"><table><thead><tr><th>Bill No</th><th>Customer</th><th>Mobile</th><th>Total Amount</th><th>Payment</th>{role === 'admin' && <><th>Expense</th><th>Profit</th></>}<th>Date</th><th>Action</th></tr></thead>
        <tbody>{data.map(b => <tr key={b.id}><td><b>{b.billNo}</b></td><td>{b.customer.name}</td><td>{b.customer.mobile}</td><td>{money(b.total ?? b.subtotal)}</td><td>{b.payment}</td>{role === 'admin' && <><td>{money(b.expense)}</td><td>{money(b.profit)}</td></>}<td>{b.date}</td><td><div className="row-actions"><button title="View bill" onClick={() => setViewBill(b)}><Eye size={15} /></button><button title="Print bill" onClick={() => printBill(b)}><Printer size={15} /></button><button title="Copy bill for WhatsApp" onClick={() => copyBill(b)}><MessageCircle size={15} /></button><button className="danger" title="Delete" onClick={() => removeBill(b.id)}><Trash2 size={15} /></button></div></td></tr>)}</tbody>
      </table>{!data.length && <p className="empty">No bills found for this date range.</p>}</div>
    </Panel>

    {viewBill && <div className="overlay"><div className="receipt-modal"><div className="receipt-actions no-print"><button className="primary-btn" onClick={() => printBill(viewBill)}><Printer /> Print / Save PDF</button><button className="wa-btn" onClick={() => copyBill(viewBill)}><MessageCircle /> Copy for WhatsApp</button><button onClick={() => setViewBill(null)}><X /></button></div>
      <div className="receipt print-report"><h2>Travel Story</h2><p>SALES RECEIPT</p><hr /><p>Bill No: <b>{viewBill.billNo}</b></p><p>Customer: <b>{viewBill.customer.name}</b> · {viewBill.customer.mobile}</p><p>Place: {viewBill.customer.place || '-'}</p><p>Date: {viewBill.date}</p>
    ...
    <style jsx global>{`
      ...
      .bill-copy-message{display:flex;align-items:center;gap:7px;width:max-content;margin:0 0 10px;padding:7px 10px;border:1px solid #b9dfc6;background:#eaf8ee;color:#15803d;border-radius:7px;font-size:11px;font-weight:700}
