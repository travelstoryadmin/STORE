...
export default function AdminShell({children}:{children:React.ReactNode}){
 const path=usePathname();const router=useRouter();const{role,currentUser,logout}=useStore();
 ...
 return <><aside className="sidebar"><div className="brand-row"><Logo/><strong>Travel Story</strong></div><nav>{visible.map(([label,href,,Icon])=><Link key={href} href={href} className={path===href?'active':''}><Icon size={18}/>{label}{label==='Expenses'&&<small>ADMIN</small>}</Link>)}</nav><button className="logout" onClick={signOut}><LogOut size={17}/>Logout</button></aside><main className="main-area"><header className="admin-header"><div><div className="page-brand">Travel Story</div><h1>{title}</h1></div><div className="profile"><div className="avatar">{role==='admin'?'A':'S'}</div><div><b>{currentUser?.name|| (role==='admin'?'Admin':'Staff')}</b><small>{role==='admin'?'Administrator':'Staff Member'}</small></div></div></header><section className="content">{children}</section></main><Toast/></>}
